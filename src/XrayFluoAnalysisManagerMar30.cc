...
void XrayFluoAnalysisManager::book()
{
  ...
  if (phaseSpaceFlag)
    ...
    man->CreateH1("h4","Gammas leaving the sample", 300,0.,200.);
    man->CreateH1("h5","Electrons leaving the sample ",200000 ,0.,200); //
    man->CreateH1("h6","Gammas reaching the detector", 100,0.,200);
    man->CreateH1("h7","Spectrum of the incident particles", 100,0.,2000);
    man->CreateH1("h8","Protons reaching the detector", 100,0.,200);
    man->CreateH1("h9","Protons leaving the sample", 100,0.,200);
    man->CreateH1("h10","Gammas reaching the collimator", 100,0.,100.);
    man->CreateH1("h11","Gammas reaching the Window", 100,0.,100.);
          ...
          ->GetDefinition()) == G4Gamma::Definition() )
           {
             G4double gammaAtTheDetPre =
               aStep->GetPreStepPoint()->GetKineticEnergy();
             man->FillH1(10,gammaAtTheDetPre/keV);
           }
     }
           ...
           ->GetDefinition()) == G4Gamma::Definition() )
            {
              G4double gammaAtTheDetPre =
                aStep->GetPreStepPoint()->GetKineticEnergy();
              man->FillH1(11,gammaAtTheDetPre/keV);
            }
      }
